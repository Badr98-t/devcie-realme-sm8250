...
function blob_fixup() {
    case "${1}" in
        ...
        odm/etc/init/vendor.oplus.hardware.oplusSensor@1.0-service.rc)
            ...
            ;;
        odm/lib/liblvimfs_wrapper.so|odm/lib64/libCOppLceTonemapAPI.so|odm/lib64/libYTCommon.so|odm/lib64/liblvimfs_wrapper.so|odm/lib/libYTCommon.so|odm/lib/libCOppLceTonemapAPI.so)
        ...
        vendor/lib64/libssc.so|vendor/lib64/libsensorcal.so|vendor/lib64/sensors.ssc.so|vendor/lib64/libsnsdiaglog.so|vendor/lib64/libsnsapi.so|vendor/bin/sensors.qti)
            "${PATCHELF}" --replace-needed "libprotobuf-cpp-lite-3.9.1.so" "libprotobuf-cpp-full-3.9.1.so" "${2}"
            ;;
        odm/lib/libdlbdsservice_v3_6.so | odm/lib/libstagefright_soft_ddpdec.so | odm/lib/libstagefrightdolby.so | odm/lib64/libdlbdsservice_v3_6.so | vendor/lib/libstagefright_soft_qtiflacdec.so | vendor/lib64/libstagefright_soft_qtiflacdec.so | odm/lib/libstagefright_soft_ac4dec_v3_6.so | odm/lib/libstagefright_soft_ddpdec_v3_6.so | odm/lib/libcodec2_soft_ozodec.so | odm/lib/libcodec2_soft_ozoenc.so | odm/lib/libozoc2store.so | system_ext/lib/libwfdavenhancements.so | system_ext/lib64/libwfdavenhancements.so)
